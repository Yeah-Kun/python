...
def url_to_module(url):
    ...
    url = ''.join([ch if ord(ch) in range(128) else parse.quote(ch) for ch in url]) # 将非ascii编码转换成ascii
    video_host = r1(r'https?://([^/]+)/', url)
...
def get_head(url, headers={}, get_method='HEAD'):
    ...
    req.get_method = lambda: get_method # 设定请求方法为"HEAD"，类似于get请求，只不过返回的响应中没有具体的内容，用于获取报头
